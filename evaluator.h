...
class Evaluator {
 public:
  Evaluator( const string &s ) : str( s ) { // constructor receives an exp
	...
	PREC_TABLE[26].inputSymbol = 13;    // ASSIGN
	PREC_TABLE[26].topOfStack = 14;

	PREC_TABLE[27].inputSymbol = 0;    // VAR A
	PREC_TABLE[27].topOfStack = 0;

	PREC_TABLE[28].inputSymbol = 0;    // VAR B
	PREC_TABLE[28].topOfStack = 0;

	PREC_TABLE[29].inputSymbol = 0;    // VAR C
	PREC_TABLE[29].topOfStack = 0;
 ...
 private:
	...
	char tempPFVS;                            // temp variable for postFixVarStack
	NumericType var_a;                        // variable a
	NumericType var_b;                        // variable b
	NumericType var_c;                        // variable c

  Precedence PREC_TABLE[30];                // precedence table

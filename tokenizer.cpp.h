...
Token<NumericType> Tokenizer<NumericType>::getToken() {
	...
	if (getChar(ch) == true) {
		...
		case '+': 
			if (prevToken == OPAREN)//Unary plus
				return (prevToken = UN_PLUS);
			else
				return (prevToken = PLUS);

		case '-': 
			if (prevToken == OPAREN)//Unary minus
				return (prevToken = UN_MINUS);
			else
				return (prevToken = MINUS);

		case '~': return (prevToken = BIT_COMP);//Unary operation
		...
		case '=':
			...
			if (getChar(ch) == true) {
				switch (ch) {
				case '=': return (prevToken = EQUAL);
				default:// assigns a char variable an int value
					in.putback(ch);
